...
class Network(object):
    ...
    def SGD(self, training_data, n_epochs, mini_batch_size,
        ...
        if validation_data: validation_data = list(validation_data)
        #validation_accuracy = []
            ...
            for mini_batch in mini_batches:
                ...
                if (epochs == 1):
                    self.update_mini_batch( mini_batch_size, mini_batch_input, \
                                            ...
                                            fudge_factor, init=True)
                else:
                    self.update_mini_batch(mini_batch_size, mini_batch_input, \
                                        mini_batch_output, lmbda, mu, n, fudge_factor)
    ...
    def update_mini_batch(self, mini_batch_size, mini_batch_input, mini_batch_output,\
                          lmbda, mu, n, fudge_factor, init=False):
        ...
        if init:
            ...
            rms_grad_w = [np.sqrt(egw + fudge_factor) for egw in self.e_grad_w]
            rms_grad_b = [np.sqrt(egb + fudge_factor) for egb in self.e_grad_b]

            # Initializing deltas ~(with eta=1 multiplying)~
            delta_w = [-np.divide(nw*sqrt(fudge_factor), rgw) \
                        for nw, rgw in zip(nabla_w, rms_grad_w)]
            delta_b = [-np.divide(nb*sqrt(fudge_factor), rgb) \
                        for nb, rgb in zip(nabla_b, rms_grad_b)]
            ...
            rms_prev_delta_w = [np.sqrt(edw + fudge_factor) for edw in self.e_delta_w]
            rms_prev_delta_b = [np.sqrt(edb + fudge_factor) for edb in self.e_delta_b]

            # Now, calculating the actual deltas
            delta_w = [-nw*np.divide(rpdw, rgw)
                        for nw, rpdw, rgw in zip(nabla_w, rms_prev_delta_w, rms_grad_w)]
            delta_b = [-nb*np.divide(rpdb, rgb)
                        for nb, rpdb, rgb in zip(nabla_b, rms_prev_delta_b, rms_grad_b)]

        # Updating the parameters (regularization without eta)
        self.weights = [(1-(lmbda/n))*w + dw
    ...
    def backprop(self, mini_batch_input, mini_batch_output):
        ...
        nabla_b[-1] = delta.sum(axis=1).reshape(self.sizes[-1], 1)
        ...
        for l in range(2, self.num_layers):
            ...
            nabla_b[-l] = delta.sum(axis=1).reshape(self.sizes[-l], 1)
